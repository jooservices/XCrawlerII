...
final class OneFourOneJavClientTest extends TestCase
{
    ...
    {
        $response = $this->getMockResponseWrapper('141jav_new.html');
        $mockHttp = Mockery::mock(HttpClientInterface::class);
        $mockHttp->shouldReceive('request')
            ->once()
            ->with('GET', OneFourOneJavClient::BASE_URI . '/new', Mockery::on(function (array $options): bool {
        ...
        $client = new OneFourOneJavClient($coreClient);
    ...
    {
        $response = $this->getMockResponseWrapper('141jav_2025-03-03.html');
        $mockHttp = Mockery::mock(HttpClientInterface::class);
        $mockHttp->shouldReceive('request')
            ->once()
            ->with('GET', OneFourOneJavClient::BASE_URI . '/2025/03/03', Mockery::on(function (array $options): bool {
                return isset($options['headers']['User-Agent']) && $options['headers']['User-Agent'] !== '';
            }))
            ->andReturn($response);
        $mockFactory = Mockery::mock(ClientFactory::class);
    ...
    {
        $response = $this->getMockResponseWrapper('141jav_popular.html');
        $mockHttp = Mockery::mock(HttpClientInterface::class);
        $mockHttp->shouldReceive('request')
            ->once()
            ->with('GET', OneFourOneJavClient::BASE_URI . '/popular', Mockery::on(function (array $options): bool {
                return isset($options['headers']['User-Agent']) && $options['headers']['User-Agent'] !== '';
            }))
            ->andReturn($response);
        $mockFactory = Mockery::mock(ClientFactory::class);
    ...
    public function test_get_throws_when_underlying_client_throws(): void
    {
        ...
        $mockHttp->shouldReceive('request')
            ->once()
            ->andThrow(new \GuzzleHttp\Exception\ConnectException('Connection refused', new \GuzzleHttp\Psr7\Request('GET', OneFourOneJavClient::BASE_URI)));
        $mockFactory = Mockery::mock(ClientFactory::class);
        $mockFactory->shouldReceive('create')->andReturn($mockHttp);
